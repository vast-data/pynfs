...
import os
import random
# Make our bundled lib/ importable whether run from the source dir or via a
# symlink (e.g. /usr/bin/testserver.py -> .../nfs3/testserver.py). Using
# realpath() resolves the symlink so lib/ is found in the real package dir.
_here = os.path.dirname(os.path.realpath(__file__))
if os.path.isfile(os.path.join(_here, 'lib', 'testmod.py')):
...
def scan_options(p):
    ...
    p.add_option("--nfsvers", default='3',
                 help="NFS version to test (only 3 is supported here)")
...
def main():
    ...
    # This is the nfs3/NSM/NLM tester; only NFS version 3 is supported here.
    # (NFSv4 lives in the sibling nfs4.1/ tree.)
    if opt.nfsvers != '3':
        p.error("Unsupported NFS version: %s (this tester only supports 3)" %
                opt.nfsvers)
    ...
    if opt.security not in valid:
        p.error("Unknown security: %s\nValid flavors are %s" %
                (opt.security, str(list(valid.keys()))))
