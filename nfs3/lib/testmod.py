...
def printresults(tests, opts, file=None):
    failures = 0
    NOTRUN, OMIT, SKIP, FAIL, WARN, PASS, TOOLONG, TOOLONGFAIL = list(range(8))
    count = [0] * 8
    for t in tests:
    ...
    print("Of those: %i Skipped, %i Failed, %i Warned," \
                   "%i Passed, %i TooLongPass, %i TooLongFail" % \
                   (count[SKIP], count[FAIL], count[WARN], count[PASS], \
                    count[TOOLONG], count[TOOLONGFAIL]), file=file)
    ...
    print("*" * 50, file=file)
    print("Isilon results:", file=file)
    for t in tests:
        ...
        if t.result == TEST_FAIL or t.result == TEST_TOOLONGFAIL:
            if t.code in EXPECTED_FAILURES:
                ...
                print("Unexpected test FAILURE: %s (expected WARNING)" % t.code, file=file)
                if not ignore_results:
                    failures += 1
                else:
                    print("Ignoring FAILURE for test %s" % t.code, file=file)
            else:
                print("Unexpected test FAILURE: %s (expected PASS)" % t.code, file=file)
                if not ignore_results:
                    failures += 1
                else:
                    print("Ignoring FAILURE for test %s" % t.code, file=file)
            ...
            if t.code in EXPECTED_WARNINGS:
                ...
                print("Unexpected test WARNING: %s (expected FAILURE)" % t.code, file=file)
                if not ignore_results:
                    failures += 1
                else:
                    print("Ignoring WARNING for test %s" % t.code, file=file)
            else:
                print("Unexpected test WARNING: %s (expected PASS)" % t.code, file=file)
                if not ignore_results:
                    failures += 1
                else:
                    print("Ignoring WARNING for test %s" % t.code, file=file)
            ...
            if t.code in EXPECTED_FAILURES:
                print("Unexpected test PASS: %s (expected FAILURE)" % t.code, file=file)
                if not ignore_results:
                    failures += 1
                else:
                    print("Ignoring PASS for test %s" % t.code, file=file)
            elif t.code in EXPECTED_WARNINGS:
                print("Unexpected test PASS: %s (expected WARNING)" % t.code, file=file)
                if not ignore_results:
                    failures += 1
                else:
                    print("Ignoring PASS for test %s" % t.code, file=file)
