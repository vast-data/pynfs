...
class post_op_fh3:
    ...
    switch = property(lambda s: {const.TRUE:s.handle,const.FALSE:None,}[s.present])
...
class set_uint32:
    ...
    switch = property(lambda s: {const.TRUE:s.val,}.get(s.set, None))
...
class set_time:
    ...
    switch = property(lambda s: {const.SET_TO_CLIENT_TIME:s.time,}.get(s.set, None))
...
class diropres3:
    ...
    switch = property(lambda s: {const.NFS3_OK:s.resok,}.get(s.status, s.resfail))
...
class wccstat3:
    ...
    #         wcc_data wcc;
    # };
    def __init__(self, status=None, wcc=None):
        self.status = status
        self.wcc = wcc

    switch = property(lambda s: {-1:None,}.get(s.status, s.wcc))
...
class getattr3res:
    ...
    switch = property(lambda s: {const.NFS3_OK:s.attributes,}.get(s.status, None))
...
class sattrguard3:
    ...
    switch = property(lambda s: {const.TRUE:s.ctime,const.FALSE:None,}[s.check])
...
class readlink3resok:
    ...
    def __getattr__(self, attr):
        return getattr(self.symlink_attributes, attr)
...
class read3resok:
    ...
    def __getattr__(self, attr):
        return getattr(self.file_attributes, attr)
...
class write3resok:
    ...
    def __getattr__(self, attr):
        return getattr(self.file_wcc, attr)
...
class createhow3:
    ...
    switch = property(lambda s: {const.UNCHECKED:s.obj_attributes,const.GUARDED:s.obj_attributes,const.EXCLUSIVE:s.verf,}[s.mode])
...
class mknoddata3:
    ...
    switch = property(lambda s: {const.NF3CHR:s.device,const.NF3BLK:s.device,const.NF3SOCK:s.pipe_attributes,const.NF3FIFO:s.pipe_attributes,}.get(s.type, None))
...
class rename3args:
    ...
    def __repr__(self):
        ...
        if self.from_dir is not None:
            out += ['from_dir=%s' % repr(self.from_dir)]
        if self.to_dir is not None:
            out += ['to_dir=%s' % repr(self.to_dir)]
...
class rename3res:
    ...
    #         rename3wcc res;
    # };
    def __init__(self, status=None, res=None):
        self.status = status
        self.res = res

    switch = property(lambda s: {-1:None,}.get(s.status, s.res))
