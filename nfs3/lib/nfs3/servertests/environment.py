...
class Environment(testmod.Environment):
    ...
    def _get_security(self, opts):
        if opts.security == 'none':
            ...
            raise RuntimeError('Bad security %s' % opts.security)
    ...
    def _maketree(self):
        ...
        res = c.mkdir(mnt_fh, c.homedir, dir_mode_set=1, dir_mode_val=0o777)
...
def check(res, stat=NFS3_OK, msg=None, warnlist=[]):
    if res.status == stat:
        if not (debug_fail and msg):
            return
    if type(stat) is str:
        raise RuntimeError("You forgot to put 'msg=' in front of check's string arg")
